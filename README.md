...
1. Download the highest-numbered `EquaSolve_V1_3_*.html` in this repository — currently **`EquaSolve_V1_3_113.html`**. This is the same build the live demo serves.
...
`Archive/` — all 78 versions from V1.234 (April 2026) through V1.3.113, under their
original filenames. Each is also a git tag dated from its original release.

```bash
ls Archive/                       # every build ever released
git tag                           # v1.234 ... v1.3.113, in release order

# what changed between two releases
git diff --no-index Archive/EquaSolve_V1_3_112.html Archive/EquaSolve_V1_3_113.html
```
...
1. The version number (currently V1.3.113)
