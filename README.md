...
1. Download the highest-numbered `EquaSolve_V1_3_*.html` in this repository — currently **`EquaSolve_V1_3_108.html`**. This is the same build the live demo serves.
...
| **v1.3.108** — the current release | [10.5281/zenodo.22687818](https://doi.org/10.5281/zenodo.22687818) |
...
`Archive/` — all 73 versions from V1.234 (April 2026) through V1.3.108, under their
original filenames. Each is also a git tag dated from its original release.

```bash
ls Archive/                       # every build ever released
git tag                           # v1.234 ... v1.3.108, in release order

# what changed between two releases
git diff --no-index Archive/EquaSolve_V1_3_107.html Archive/EquaSolve_V1_3_108.html
```
...
1. The version number (currently V1.3.108)
